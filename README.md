...
| :---: | :---: | --- |
| `2.2.0` | `2026-06-13` | <ul><li>**Celengan Syahid — Full Revamp** — Complete redesign of the donation flow: campaign listing, detail page, donation form, and payment status page with improved UI, dark mode support, and mobile responsiveness</li><li>**BisaTopup QRIS Gateway** — New QRIS payment method via BisaTopup integration; QRIS QR code rendered on payment status page with auto-polling every 5 seconds; payment status URL automatically saved as `payment_link` for admin tracking</li><li>**reCAPTCHA Enterprise Migration** — Migrated from deprecated reCAPTCHA v2 to Google Cloud reCAPTCHA Enterprise (score-based, invisible); no checkbox shown to users; server-side verification via Enterprise Assessments API with configurable score threshold</li><li>**Admin Donation — Payment Method Column** — New `Payment Method` column in donation index with text search filter; shows gateway method (QRIS, EWALLET, etc.)</li><li>**Admin Donation Detail — Refactored View** — Donation detail page redesigned to match the admin comment view style (page-title, section-title, card layout, back button at bottom)</li><li>**Donation Preset Nominations** — Added small preset buttons Rp1.000, Rp2.000, Rp5.000 to the donation form; minimum donation reduced from Rp10.000 to Rp1.000</li><li>**Audit Log UI Cleanup** — Removed Filter and Clear buttons from the audit log page; filtering now happens automatically via select2 change and search input debounce</li><li>**QRIS Scan Dark Mode** — QRIS QR code display on payment status page now supports dark mode</li><li>**Prevent Duplicate WA Notification** — Fixed double WhatsApp invoice notification when BisaTopup callback fires multiple times</li><li>**Donation Throttle** — Increased donation form rate limit from 10 to 30 submissions per minute to prevent false 429 errors during normal usage</li><li>**Queue Job Attempts Column** — Widened `attempts` column in `tr_job_queues` table to prevent overflow on high-retry jobs</li><li>**Validation Failure Logging** — Donation form validation rejections (including reCAPTCHA failures) are now logged with field-level error detail and requester IP</li></ul> |
